...
describe('Starting tests for angular-app', function() {
  ...
  it('network-name should be voting-network@0.0.2-deploy.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('voting-network@0.0.2-deploy.1.bna');
    });
  });
  ...
  it('navbar-brand should be angular-app',() => {
    ...
    it('canVote component should be loadable',() => {
      page.navigateTo('/canVote');
      browser.findElement(by.id('assetName'))
      .then((assetName) => {
        return assetName.getText();
      })
      .then((txt) => {
        expect(txt).toBe('canVote');
      });
    });

    it('canVote table should have 3 columns',() => {
      page.navigateTo('/canVote');
    ...
    it('candidate table should have 3 columns',() => {
      page.navigateTo('/candidate');
      element.all(by.css('.thead-cols th')).then(function(arr) {
        expect(arr.length).toEqual(3); // Addition of 1 for 'Action' column
      });
